...
public class EditSurveyServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            surveydata sdta=dao.getServeyUser(formdata.getAadharcard());
            
            

            HttpSession s = request.getSession();
            s.setAttribute("currentsurveydata", sdta);
        

            String oldimage = sdta.getReportname();

            if (reportname != "") {
                formdata.setReportname(reportname);
            } else {
                formdata.setReportname(oldimage);
            }

            if (dao.saveEditSurveyData(formdata)) {
               

                    
                    

                    String path = request.getRealPath("/") + "surveyreports-pic" + File.separator + formdata.getReportname();
                    String oldpath = request.getRealPath("/") + "surveyreports-pic" + File.separator + oldimage;

                    if (reportname != "") {
//                out.print("not updated");

                        picUpdate.deteteFile(oldpath);
